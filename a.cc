...
int dijkstra(const G& g, const VI& v, int ini, int fin) {
    ...
    while (not pq.empty()) {
        ...
        if (not vis[next]) {
            ...
            for (auto k : g[next]) {
                int min_cost = k.second + dist[next];
                if (k.first != fin) min_cost += v[k.first];
                if (min_cost < dist[k.first]) {
                    dist[k.first] = min_cost;
                    pq.push({min_cost, k.first});
                }
            }
        }
    }
